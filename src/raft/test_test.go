...
//	fmt.Printf("********************leader1:%v disconnect********************\n", leader1)
	...
	cfg.connect(leader1)
//	fmt.Printf("********************leader1:%v reconnect********************\n", leader1)
...
//	fmt.Printf("********************other server:%v disconnect********************\n", (leader2 + 1) % servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
//	fmt.Printf("********************other server:%v reconnect********************\n", (leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
//	fmt.Printf("********************leader2:%v reconnect********************\n", leader2)
	cfg.checkOneLeader()
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
//	fmt.Printf("********************leader:%v disconnect********************\n", leader1)
	cfg.disconnect(leader1)
...
func TestBackup2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
//	fmt.Printf("********************disconnect server:%v, %v, %v********************\n", (leader1 + 2) % servers, (leader1 + 3) % servers, (leader1 + 4) % servers)
	cfg.disconnect((leader1 + 2) % servers)
	cfg.disconnect((leader1 + 3) % servers)
	cfg.disconnect((leader1 + 4) % servers)

	// submit lots of commands that won't commit
//	fmt.Printf("********************add never commit log to %v********************\n", leader1)
	for i := 0; i < 50; i++ {
		cfg.rafts[leader1].Start(rand.Int())
	}

	time.Sleep(RaftElectionTimeout / 2)
//	fmt.Printf("********************disconnect server:%v, %v********************\n", (leader1 + 0) % servers, (leader1 + 1) % servers)
	cfg.disconnect((leader1 + 0) % servers)
	cfg.disconnect((leader1 + 1) % servers)

//	fmt.Printf("********************reconnect server:%v, %v, %v********************\n", (leader1 + 2) % servers, (leader1 + 3) % servers, (leader1 + 4) % servers)
	// allow other partition to recover
	cfg.connect((leader1 + 2) % servers)
	cfg.connect((leader1 + 3) % servers)
	cfg.connect((leader1 + 4) % servers)

//	fmt.Printf("********************add successful logs to new group********************\n")
...
//	fmt.Printf("********************reconnect all server********************\n")
	// now everyone
	for i := 0; i < servers; i++ {
		cfg.connect(i)
	}
